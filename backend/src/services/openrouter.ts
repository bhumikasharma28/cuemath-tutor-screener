import { TutorProfile, ChatMessage, EvaluationReport } from '../types';
...
function buildSystemPrompt(profile: TutorProfile): string {
...
export async function getInitialMessage(profile: TutorProfile): Promise<string> {
...
export function parseEvaluation(text: string): EvaluationReport | null {
  const match = text.match(/<evaluation>([\s\S]*?)<\/evaluation>/);
  if (!match) return null;
  try {
    return JSON.parse(match[1].trim()) as EvaluationReport;
  } catch {
    return null;
  }
}

export function stripEvaluationBlock(text: string): string {
